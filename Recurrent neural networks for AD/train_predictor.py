...
def train(args, model, train_dataset,epoch):
    ...
    with torch.enable_grad():
        ...
        for batch, i in enumerate(range(0, train_dataset.size(0) - 1, args.bptt)):
            ...
            loss1 = criterion(outSeq1.reshape(args.batch_size,-1), targetSeq.reshape(args.batch_size,-1)) # view

            '''Loss2: Teacher forcing loss'''
            outSeq2, hidden, hids2 = model.forward(inputSeq, hidden, return_hiddens=True)
            loss2 = criterion(outSeq2.reshape(args.batch_size, -1), targetSeq.reshape(args.batch_size, -1)) # view
